...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    await queryInterface.createTable('AICommentaries', {
      id: {
    ...
    // Add indexes
    await queryInterface.addIndex('AICommentaries', ['storyId', 'tagId', 'version'], {
      unique: true,
      name: 'unique_story_tag_commentary_version'
    });

    await queryInterface.addIndex('AICommentaries', ['storyId', 'tagId', 'isCurrent'], {
      name: 'idx_story_tag_current_commentary'
    });

    await queryInterface.addIndex('AICommentaries', ['storyId'], {
      name: 'idx_ai_commentary_story'
    });

    await queryInterface.addIndex('AICommentaries', ['tagId'], {
      name: 'idx_ai_commentary_tag'
    });

    await queryInterface.addIndex('AICommentaries', ['triggerType'], {
      name: 'idx_ai_commentary_trigger'
    });
  },

  down: async (queryInterface, Sequelize) => {
    await queryInterface.dropTable('AICommentaries');
  }
}; 